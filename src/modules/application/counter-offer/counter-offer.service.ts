...
export class CounterOfferService {
  ...
  async createCounterOffer(
    ...
    if (job.user_id === userId)
      throw new ForbiddenException('You can not counter offer your own job');
    ...
    await this.counterOfferNotificationService.notifyUserAboutCounterOffer(
      counterOffer.id,
    );
    return {
      success: true,
  ...
  async acceptCounterOffer(
    ...
    const prismaTransaction = await this.prisma.$transaction(async (prisma) => {
      ...
      if (counterOffer.helper_id === userId)
      ...
      const paymentIntent =
        await this.stripeMarketplaceService.createMarketplacePaymentIntent({
          ...
          helperStripeAccountId:
            updatedJob.assigned_helper.stripe_connect_account_id,
  ...
  async helperAcceptsJob(helperId: string, jobId: string) {
    ...
    return {
      success: true,
      message:
        'Direct acceptance submitted as a counter offer. Awaiting user decision.',
      offer,
      job: dto,
    };
  }
  ...
  async declineCounterOffer(userId: string, counterOfferId: string) {
    ...
    if (!counterOffer) throw new NotFoundException('Counter offer not found');
    if (counterOffer.job.user_id !== userId)
      throw new UnauthorizedException(
  ...
  async getAcceptedOfferByJobId(userId: string, jobId: string) {
    const job = await this.prisma.job.findFirst({
      where: {
        AND: [
          { id: jobId, job_status: 'confirmed' },
          {
    ...
    if (!job) throw new NotFoundException('Accepted offer not found');
    const isOwner = job.user_id === userId;
    const isHelper = job.assigned_helper_id === userId;

    let customMessage = '';

    if (isOwner) customMessage = 'Your job has been Confirmed';
    if (isHelper)
      customMessage = 'Congratulations! Your Counter Offer has been Accepted';

    return {
      success: true,
      message: customMessage,
      data: {
        job: this.jobService.mapToResponseDto(job),
      },
    };
  }

  async getRecentOffer(userId: string) {
    const recentCounterOffer = await this.prisma.counterOffer.findFirst({
      where: {
        OR: [
          {
            job: {
              user_id: userId,
              job_status: 'counter_offer',
              deleted_at: null,
            },
          },
          {
    ...
    if (!recentCounterOffer) {
      throw new NotFoundException('Recent offer not found');
    }

    return {
      success: true,
      message: 'Recent offer fetched successfully',
      data: {
        counterOffer: {
          id: recentCounterOffer.id,
          amount: recentCounterOffer.amount,
          type: recentCounterOffer.type,
          note: recentCounterOffer.note,
          created_at: recentCounterOffer.created_at,
          helper: recentCounterOffer.helper,
        },
        job: this.jobService.mapToResponseDto(recentCounterOffer.job),
      },
    };
  }

  async getOffer(userId: string, jobId: string) {
    const offers = await this.prisma.counterOffer.findMany({
      where: {
        job_id: jobId,
        job: {
          user_id: userId,
